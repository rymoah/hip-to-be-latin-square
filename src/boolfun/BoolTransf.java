...
public class BoolTransf {
    ...
    /**
     * Computes the Haar Transform of a boolean function using the Fast Haar
     * Transform algorithm, which requires O(N) operations (N=2^n is the length
     * of the truth table). The method directly computes the spectrum
     * in the original vector, which must be sorted in bit-reverse order. The
     * method must be called with the initial parameters (vector, vector.length).
     * The reference for the algorithm is Stankovic, Falkowski, "The Haar
     * wavelet transform: its status and achievements", p. 32.
     * 
     * @param vector an array of integers representing the boolean function.
     * @param end    the index of the truth table where to stop computations.
     */
    public static void calcFHT(int[] vector, int end) {
        
        int half = end/2;
        
        //Main cycle: split vector in two parts (v0 and v1),
        //update v0 as v0=v0+v1 and v1=v0-v1.
    ...
    /**
     * Computes the number of nonzero linear structure in a boolean function
